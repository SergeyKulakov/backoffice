...
import { updateSuggestedGeography } from "../../../thunks/suggestedGeography";
...
export const EditForm: FC<IProps> = ({ data }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, helpers): Promise<void> => {
      try {
        await dispatch(updateSuggestedGeography(values)).unwrap();
        helpers.setStatus({ success: true });
        helpers.setSubmitting(false);
        toast.success("Suggested geography updated");
  ...
  return (
    <form onSubmit={formik.handleSubmit}>
      <Card>
        <CardHeader sx={{ mb: 4 }} title="Edit suggested geography" />
        <CardContent sx={{ pt: 0 }}>
