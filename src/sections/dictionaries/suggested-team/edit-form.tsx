...
import { updateSuggestedTeam } from "../../../thunks/suggested-team";
...
export const EditForm: FC<IProps> = ({ data }) => {
  ...
  const formik = useFormik({
    ...
    onSubmit: async (values, helpers): Promise<void> => {
      try {
        await dispatch(updateSuggestedTeam(values)).unwrap();
        helpers.setStatus({ success: true });
        helpers.setSubmitting(false);
        toast.success("Suggested team updated");
  ...
  return (
    <form onSubmit={formik.handleSubmit}>
      <Card>
        <CardHeader sx={{ mb: 4 }} title="Edit suggested team" />
        <CardContent sx={{ pt: 0 }}>
